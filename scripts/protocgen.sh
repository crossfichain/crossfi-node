...
for dir in $proto_dirs; do
  ...
  for file in $proto_files; do
    # Check if the go_package in the file is pointing to mineplex
    if grep -q "option go_package.*mineplex" "$file"; then
...
cp -r github.com/mineplexio/mineplex-2-node/x/* x/
rm -rf github.com
