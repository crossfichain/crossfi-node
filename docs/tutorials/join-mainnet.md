...
# Join the Mineplex Chain Mainnet

The current Mineplex Chain mainnet, `mineplex-1`.
...
The current Mineplex Chain mainnet `mineplex-1`. Visit the [migration section](https://github.com/cosmos/mineplex-chain/tree/main/docs/migration) of the Hub's docs for more information on previous chain migrations.

## Explorers

There are many explorers for the Mineplex Chain. For reference while setting up a node, here are a few recommendations:
...
- Ensure Mineplex-chain is properly installed. See the [installation guide](https://hub.cosmos.network/main/getting-started/installation.html) for a walkthrough.
- Follow the [configuration guide](#General-Configuration) to intialize and prepare the node to sync with the network.

## Hardware

Running a full archive node can be resource intensive as the full current `mineplex-1` can be is over `1TB`. For those who wish to run state sync or use quicksync, the following hardware configuration is recommended:
...
Make sure to walk through the basic setup and configuration. Operators will need to initialize `mineplex-chaind`, download the genesis file for `mineplex-1`, and set persistent peers and/or seeds for startup.

### Initialize Chain

Choose a custom moniker for the node and initialize. By default, the `init` command creates the `~/.mineplex-chain` directory with subfolders `config` and `data`. In the `/config` directory, the most important files for configuration are `app.toml` and `config.toml`.

```bash
mineplex-chaind init <custom-moniker>
```
...
The `moniker` can be edited in the `~/.mineplex-chain/config/config.toml` file:
...
Once the node is initialized, download the genesis file and move to the `/config` directory of the Mineplex-chain home directory.

```bash
wget TODO
gzip -d genesis.mineplex-1.json.gz
mv genesis.mineplex-1.json ~/.mineplex-chain/config/genesis.json
```

### Seeds & Peers

Upon startup the node will need to connect to peers. If there are specific nodes a node operator is interested in setting as seeds or as persistent peers, this can be configured in `~/.mineplex-chain/config/config.toml`
...
Node operators can optionally download the [Quicksync address book]( https://quicksync.io/addrbook.cosmos.json). Make sure to move this to `~/.mineplex-chain/config/addrbook.json`.

### Gas & Fees

On Mineplex Chain mainnet, the accepted denom is `mpx`, where `1 MPX = 1.000.000mpx`

Transactions on the Mineplex Chain network need to include a transaction fee in order to be processed. This fee pays for the gas required to run the transaction. The formula is the following:
...
**For mainnet, the recommended `gas-prices` is `0.0025mpx`.**

A full-node keeps unconfirmed transactions in its mempool. In order to protect it from spam, it is better to set a `minimum-gas-prices` that the transaction must meet in order to be accepted in the node's mempool. This parameter can be set in  `~/.mineplex-chain/config/app.toml`.
...
The initial recommended `min-gas-prices` is `0.0025mpx`, but this can be changed later.
...
To set pruning, adjust the `pruning` parameter in the `~/.mineplex-chain/config/app.toml` file.
...
In `~/.mineplex-chain/config/app.toml`
...
Passing a flag when starting `mineplex-chain` will always override settings in the `app.toml` file. To change the node's pruning setting to `everything` mode then pass the `---pruning everything` flag when running `mineplex-chaind start`.
...
By default, the REST API is disabled. To enable the REST API, edit the `~/.mineplex-chain/config/app.toml` file, and set `enable` to `true` in the `[api]` section.
...
By default, gRPC is enabled on port `9090`. The `~/.mineplex-chain/config/app.toml` file is where changes can be made in the gRPC section. To disable the gRPC endpoint, set `enable` to `false`. To change the port, use the `address` parameter.
...
If a node operator wishes to run a full node, it is possible to start from scratch but will take a significant amount of time to catch up. Node operators not concerned with rebuilding original state from the beginning of `mineplex-1` can also leverage [Quicksync](#Quicksync)'s available archive history.
...
It is possible to sync from previous versions of the Mineplex Chain. See the matrix below for the correct `mineplex-chain` version. See the [mainnet archive](https://github.com/cosmos/mainnet) for historical genesis files.

| Chain Id      | Mineplex Version |
| -----------   |------------------|
| `mineplex-1` | `v1.0.0`         |

##### Getting Started

Start Mineplex-chain to begin syncing with the `skip-invariants` flag. For more information on this see [Verify Mainnet](#Verify-Mainnet).

```bash
mineplex-chaind start --x-crisis-skip-assert-invariants
```
...
Start Mineplex-chain to begin state sync. It may take take some time for the node to acquire a snapshot, but the command and output should look similar to the following:
...
Once state sync successfully completes, the node will begin to process blocks normally. If state sync fails and the node operator encounters the following error:  `State sync failed err="state sync aborted"`, either try restarting `mineplex-chaind` or running `mineplex-chaind unsafe-reset-all` (make sure to backup any configuration and history before doing this).

### Quicksync

Quicksync.io offers several  daily snapshots of the Mineplex Chain with varying levels of pruning (`archive` 1.4TB, `default` 540GB, and `pruned` 265GB). For downloads and installation instructions, visit the [Cosmos Quicksync guide](https://quicksync.io/networks/cosmos.html).

## Snapshots

Saving and serving snapshots helps nodes rapidly join the network. Snapshots are now enabled by default effective `1/20/21`.

While not advised, if a node operator needs to customize this feature, it can be configured in `~/.mineplex-chain/config/app.toml`. The Mineplex Chain recommends setting this value to match `pruning-keep-every` in `config.toml`.
...
**See all [Mineplex-chain Releases](https://github.com/cosmos/mineplex-chain/releases)**

The most up to date release of Mineplex-chain is [`V8.0.0`](https://github.com/cosmos/mineplex-chain/releases/tag/v8.0.0). For those that want to use state sync or quicksync to get their node up to speed, starting with the most recent version of Mineplex-chain is sufficient.

To sync an archive or full node from scratch, it is important to note that you must start with [`V4.2.1`](https://github.com/cosmos/mineplex-chain/releases/tag/v4.2.1) and proceed through two different upgrades Delta at block height `6910000` and Vega at block height `8695000`.

The process is summarized below but make sure to follow the manual upgrade instructions for each release:

**[Delta Instructions](https://github.com/cosmos/mineplex-chain/blob/main/docs/migration/mineplex-1-delta-upgrade.md#Upgrade-will-take-place-July-12,-2021)**
...
Make sure to save a backup of `~/.mineplex-chain` in case rolling back is necessary.

Install Mineplex-chain [`V5.0.0`](https://github.com/cosmos/mineplex-chain/releases/tag/v5.0.0) and restart the daemon.

**[Vega Instructions](https://github.com/cosmos/mineplex-chain/blob/main/docs/migration/mineplex-1-vega-upgrade.md)**
...
Again, make sure to backup `~/.mineplex-chain`

Install Mineplex-chain [`V6.0.0`](https://github.com/cosmos/mineplex-chain/releases/tag/v6.0.0) and restart the daemon.
...
[Unit]
Description=Mineplex-chain Daemon
After=network-online.target

[Service]
User=$USER
ExecStart=$(which mineplex-chaind) start
Restart=always
RestartSec=3
LimitNOFILE=4096
...
Environment="DAEMON_NAME=mineplex-chaind"
Environment="DAEMON_ALLOW_DOWNLOAD_BINARIES=false"
Environment="DAEMON_RESTART_AFTER_UPGRADE=true"
```

After the `LimitNOFILE` line and replace `$(which mineplex-chaind)` with `$(which cosmovisor)`.
...
Mineplex-chain can dump the entire application state into a JSON file. This application state dump is useful for manual analysis and can also be used as the genesis file of a new network.
...
```bash
mineplex-chaind start --assert-invariants-blockly
```
...
invariant broken:
    ...
    CRITICAL please submit the following transaction:
        mineplex-chaind tx crisis invariant-broken staking supply
