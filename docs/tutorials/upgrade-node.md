...
echo "export DAEMON_NAME=mineplex-chaind" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.mineplex-chain" >> ~/.profile
source ~/.profile
```
...
cp $(which mineplex-chaind) ~/.mineplex-chain/cosmovisor/genesis/bin/

# verify the setup. 
# It should return the same version as mineplex-chaind
cosmovisor version
```

Now `mineplex-chaind` can start by running
...
First, stop your instance of `mineplex-chaind`. Next, upgrade the software:
...
```bash
cd $HOME/.mineplex-chain/config
mineplex-chaind export --for-zero-height --height=<export-height> > new_genesis.json
```
...
::: warning
If you are running a **validator node** on the mainnet, always be careful when doing `mineplex-chaind unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
