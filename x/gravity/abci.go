...
func createValsets(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper) {
	...
	if (latestValset == nil) || (lastUnbondingHeight == uint64(ctx.BlockHeight())) || significantPowerDiff {
		...
		k.SetValsetRequest(ctx, chainID)
	}
}
...
func pruneValsets(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper, params types.Params) {
	...
	lastObserved := k.GetLastObservedValset(ctx, chainID)
	...
	if lastObserved != nil && !tooEarly {
		...
		sets := k.GetValsets(ctx, chainID)
		for _, set := range sets {
			if set.Nonce < lastObserved.Nonce && set.Height < earliestToPrune {
				k.DeleteValset(ctx, chainID, set.Nonce)
				k.DeleteValsetConfirms(ctx, chainID, set.Nonce)
			}
		}
	}
}
...
func slashing(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper) {
	...
	batchSlashing(ctx, chainID, k, params)
	logicCallSlashing(ctx, chainID, k, params)
}
...
func cleanupTimedOutBatches(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper) {
	...
	for _, batch := range batches {
		if batch.BatchTimeout < ethereumHeight {
			err := k.CancelOutgoingTXBatch(ctx, chainID, batch.TokenContract, batch.BatchNonce)
			if err != nil {
...
func prepValsetConfirms(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper, nonce uint64) map[string]types.MsgValsetConfirm {
	confirms := k.GetValsetConfirms(ctx, chainID, nonce)
...
func valsetSlashing(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper, params types.Params) {
	...
	unslashedValsets := k.GetUnSlashedValsets(ctx, chainID, params.SignedValsetsWindow)

	currentBondedSet := k.StakingKeeper.GetBondedValidatorsByPower(ctx)
	unbondingValidators := getUnbondingValidators(ctx, k)

	for _, vs := range unslashedValsets {
		confirms := prepValsetConfirms(ctx, chainID, k, vs.Nonce)
		...
		k.SetLastSlashedValsetNonce(ctx, chainID, vs.Nonce)
	}
}
...
// reducing the lookup time dramatically and separating out the task of looking up
// the orchestrator for each validator
func prepBatchConfirms(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper, batch types.InternalOutgoingTxBatch) map[string]types.MsgConfirmBatch {
	confirms := k.GetBatchConfirmByNonceAndTokenContract(ctx, chainID, batch.BatchNonce, batch.TokenContract)
...
func batchSlashing(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper, params types.Params) {
	...
	currentBondedSet := k.StakingKeeper.GetBondedValidatorsByPower(ctx)
	unslashedBatches := k.GetUnSlashedBatches(ctx, chainID, maxHeight)
	for _, batch := range unslashedBatches {
		// SLASH BONDED VALIDTORS who didn't attest batch requests
		confirms := prepBatchConfirms(ctx, chainID, k, batch)
		for _, val := range currentBondedSet {
			consAddr, err := val.GetConsAddr()
			if err != nil {
				panic(err)
		...
		k.SetLastSlashedBatchBlock(ctx, chainID, batch.CosmosBlockCreated)
	}
}
...
func logicCallSlashing(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper, params types.Params) {
	...
	currentBondedSet := k.StakingKeeper.GetBondedValidatorsByPower(ctx)
	unslashedLogicCalls := k.GetUnSlashedLogicCalls(ctx, chainID, maxHeight)
	for _, call := range unslashedLogicCalls {
		...
		k.SetLastSlashedLogicCallBlock(ctx, chainID, call.CosmosBlockCreated)
	}
}
...
func pruneAttestations(ctx sdk.Context, chainID types.ChainID, k keeper.Keeper) {
	attmap, keys := k.GetAttestationMapping(ctx, chainID)
	...
	lastNonce := uint64(k.GetLastObservedEventNonce(ctx, chainID))
	var cutoff uint64
