...
func (k Keeper) CheckBadSignatureEvidence(
	...
	case *types.OutgoingTxBatch:
		return k.checkBadSignatureEvidenceInternal(ctx, chainID, subject, msg.Signature)
	case *types.Valset:
		return k.checkBadSignatureEvidenceInternal(ctx, chainID, subject, msg.Signature)
...
func (k Keeper) checkBadSignatureEvidenceInternal(ctx sdk.Context, chainID types.ChainID, subject types.EthereumSigned, signature string) error {
	...
	if k.GetPastEthSignatureCheckpoint(ctx, chainID, checkpoint) {
...
func (k Keeper) SetPastEthSignatureCheckpoint(ctx sdk.Context, chainID types.ChainID, checkpoint []byte) {
	store := ctx.KVStore(k.storeKey)
	store.Set(types.GetPastEthSignatureCheckpointKey(chainID, checkpoint), []byte{0x1})
}

// GetPastEthSignatureCheckpoint tells you whether a given checkpoint has ever existed
func (k Keeper) GetPastEthSignatureCheckpoint(ctx sdk.Context, chainID types.ChainID, checkpoint []byte) (found bool) {
	store := ctx.KVStore(k.storeKey)
	if bytes.Equal(store.Get(types.GetPastEthSignatureCheckpointKey(chainID, checkpoint)), []byte{0x1}) {
		return true
	} else {
		return false
	}
