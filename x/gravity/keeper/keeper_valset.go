...
func (k Keeper) SetValsetRequest(ctx sdk.Context, chainID types.ChainID) types.Valset {
	valset, err := k.GetCurrentValset(ctx, chainID)
	if err != nil {
		panic(err)
	}
	k.StoreValset(ctx, chainID, valset)
	k.SetLatestValsetNonce(ctx, chainID, valset.Nonce)
...
func (k Keeper) StoreValset(ctx sdk.Context, chainID types.ChainID, valset types.Valset) {
	key := types.GetValsetKey(chainID, valset.Nonce)
...
func (k Keeper) HasValsetRequest(ctx sdk.Context, chainID types.ChainID, nonce uint64) bool {
	store := ctx.KVStore(k.storeKey)
	return store.Has(types.GetValsetKey(chainID, nonce))
}

// DeleteValset deletes the valset at a given nonce from state
func (k Keeper) DeleteValset(ctx sdk.Context, chainID types.ChainID, nonce uint64) {
	ctx.KVStore(k.storeKey).Delete(types.GetValsetKey(chainID, nonce))
}
...
func (k Keeper) CheckLatestValsetNonce(ctx sdk.Context, chainID types.ChainID) bool {
	store := ctx.KVStore(k.storeKey)
	has := store.Has(types.AppendBytes(types.LatestValsetNonce, chainID.Bytes()))
	return has
}

// GetLatestValsetNonce returns the latest valset nonce
func (k Keeper) GetLatestValsetNonce(ctx sdk.Context, chainID types.ChainID) uint64 {
	if !k.CheckLatestValsetNonce(ctx, chainID) {
...
// to decrement
func (k Keeper) SetLatestValsetNonce(ctx sdk.Context, chainID types.ChainID, nonce uint64) {
	// this is purely an increasing counter and should never decrease
	if k.CheckLatestValsetNonce(ctx, chainID) && k.GetLatestValsetNonce(ctx, chainID) > nonce {
...
func (k Keeper) GetValset(ctx sdk.Context, chainID types.ChainID, nonce uint64) *types.Valset {
	store := ctx.KVStore(k.storeKey)
	bz := store.Get(types.GetValsetKey(chainID, nonce))
	if bz == nil {
		return nil
	}
...
func (k Keeper) IterateValsets(ctx sdk.Context, chainID types.ChainID, cb func(key []byte, val *types.Valset) (stop bool)) {
	prefixStore := prefix.NewStore(ctx.KVStore(k.storeKey), types.AppendBytes(types.ValsetRequestKey, chainID.Bytes()))
...
func (k Keeper) GetValsets(ctx sdk.Context, chainID types.ChainID) (out []types.Valset) {
	k.IterateValsets(ctx, chainID, func(_ []byte, val *types.Valset) bool {
		out = append(out, *val)
		return false
	})
...
func (k Keeper) GetLatestValset(ctx sdk.Context, chainID types.ChainID) (out *types.Valset) {
	latestValsetNonce := k.GetLatestValsetNonce(ctx, chainID)
	out = k.GetValset(ctx, chainID, latestValsetNonce)
	return
}

// setLastSlashedValsetNonce sets the latest slashed valset nonce
func (k Keeper) SetLastSlashedValsetNonce(ctx sdk.Context, chainID types.ChainID, nonce uint64) {
	store := ctx.KVStore(k.storeKey)
	store.Set(types.AppendBytes(types.LastSlashedValsetNonce, chainID.Bytes()), types.UInt64Bytes(nonce))
}

// GetLastSlashedValsetNonce returns the latest slashed valset nonce
func (k Keeper) GetLastSlashedValsetNonce(ctx sdk.Context, chainID types.ChainID) uint64 {
	store := ctx.KVStore(k.storeKey)
	bytes := store.Get(types.AppendBytes(types.LastSlashedValsetNonce, chainID.Bytes()))
...
func (k Keeper) SetLastUnBondingBlockHeight(ctx sdk.Context, chainID types.ChainID, unbondingBlockHeight uint64) {
	store := ctx.KVStore(k.storeKey)
	store.Set(types.AppendBytes(types.LastUnBondingBlockHeight, chainID.Bytes()), types.UInt64Bytes(unbondingBlockHeight))
}
...
func (k Keeper) GetLastUnBondingBlockHeight(ctx sdk.Context, chainID types.ChainID) uint64 {
	store := ctx.KVStore(k.storeKey)
	bytes := store.Get(types.AppendBytes(types.LastUnBondingBlockHeight, chainID.Bytes()))
...
func (k Keeper) GetUnSlashedValsets(ctx sdk.Context, chainID types.ChainID, signedValsetsWindow uint64) (out []*types.Valset) {
	lastSlashedValsetNonce := k.GetLastSlashedValsetNonce(ctx, chainID)
	blockHeight := uint64(ctx.BlockHeight())
	k.IterateValsetBySlashedValsetNonce(ctx, chainID, lastSlashedValsetNonce, func(_ []byte, valset *types.Valset) bool {
...
func (k Keeper) IterateValsetBySlashedValsetNonce(ctx sdk.Context, chainID types.ChainID, lastSlashedValsetNonce uint64, cb func([]byte, *types.Valset) bool) {
	prefixStore := prefix.NewStore(ctx.KVStore(k.storeKey), types.AppendBytes(types.ValsetRequestKey, chainID.Bytes()))
	// Consider all valsets, including the most recent one
	cutoffNonce := k.GetLatestValsetNonce(ctx, chainID) + 1
...
// it or save
func (k Keeper) GetCurrentValset(ctx sdk.Context, chainID types.ChainID) (types.Valset, error) {
	...
	valsetNonce := k.GetLatestValsetNonce(ctx, chainID) + 1
...
func (k Keeper) GetValsetConfirm(ctx sdk.Context, chainID types.ChainID, nonce uint64, validator sdk.AccAddress) *types.MsgValsetConfirm {
	store := ctx.KVStore(k.storeKey)
	if err := sdk.VerifyAddressFormat(validator); err != nil {
		ctx.Logger().Error("invalid validator address")
		return nil
	}
	entity := store.Get(types.GetValsetConfirmKey(chainID, nonce, validator))
	if entity == nil {
		return nil
	}
...
func (k Keeper) SetValsetConfirm(ctx sdk.Context, chainID types.ChainID, valsetConf types.MsgValsetConfirm) []byte {
	store := ctx.KVStore(k.storeKey)
	addr, err := sdk.AccAddressFromBech32(valsetConf.Orchestrator)
	if err != nil {
		panic(err)
	}
	key := types.GetValsetConfirmKey(chainID, valsetConf.Nonce, addr)
	store.Set(key, k.cdc.MustMarshal(&valsetConf))
	return key
}

// GetValsetConfirms returns all validator set confirmations by nonce
func (k Keeper) GetValsetConfirms(ctx sdk.Context, chainID types.ChainID, nonce uint64) (confirms []types.MsgValsetConfirm) {
	store := ctx.KVStore(k.storeKey)
	prefix := types.GetValsetConfirmNoncePrefix(chainID, nonce)
...
func (k Keeper) IterateValsetConfirms(ctx sdk.Context, chainID types.ChainID, cb func(key []byte, confirms []types.MsgValsetConfirm, nonce uint64) (stop bool)) {
	prefixStore := prefix.NewStore(ctx.KVStore(k.storeKey), types.AppendBytes(types.ValsetConfirmKey, chainID.Bytes()))
	iter := prefixStore.ReverseIterator(nil, nil)
	defer iter.Close()
...
func (k Keeper) DeleteValsetConfirms(ctx sdk.Context, chainID types.ChainID, nonce uint64) {
	store := ctx.KVStore(k.storeKey)
	for _, confirm := range k.GetValsetConfirms(ctx, chainID, nonce) {
		orchestrator, err := sdk.AccAddressFromBech32(confirm.Orchestrator)
		if err == nil {
			confirmKey := types.GetValsetConfirmKey(chainID, nonce, orchestrator)
