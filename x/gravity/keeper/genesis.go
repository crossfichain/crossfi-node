...
func InitGenesis(ctx sdk.Context, k Keeper, data types.GenesisState) {
	...
	for _, chain := range data.Chains {
		...
		k.setID(ctx, chain.GravityNonces.LastTxPoolId, types.AppendBytes(types.KeyLastTXPoolID, chainID.Bytes()))
		k.setID(ctx, chain.GravityNonces.LastBatchId, types.AppendBytes(types.KeyLastOutgoingBatchID, chainID.Bytes()))
		...
		valsetReward := k.GetParamsForChain(ctx, chainID).ValsetReward
	...
	for _, chain := range p.Chains {
		...
		chains = append(chains, &types.GravityChain{
			GravityNonces: types.GravityNonces{
				...
				LastTxPoolId:              k.getID(ctx, types.AppendBytes(types.KeyLastTXPoolID, chainID.Bytes())),
				LastBatchId:               k.getID(ctx, types.AppendBytes(types.KeyLastOutgoingBatchID, chainID.Bytes())),
			},
			Valsets:            valsets,
