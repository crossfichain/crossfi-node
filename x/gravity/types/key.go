...
// prefix    nonce
// [0x0][0 0 0 0 0 0 0 1]
func GetValsetKey(chainID ChainID, nonce uint64) []byte {
	return AppendBytes(ValsetRequestKey, chainID.Bytes(), UInt64Bytes(nonce))
}

// GetValsetConfirmNoncePrefix returns the following format
// prefix   nonce
// [0x0][0 0 0 0 0 0 0 1]
func GetValsetConfirmNoncePrefix(chainID ChainID, nonce uint64) []byte {
	return AppendBytes(ValsetConfirmKey, chainID.Bytes(), UInt64Bytes(nonce))
}
...
func GetValsetConfirmKey(chainID ChainID, nonce uint64, validator sdk.AccAddress) []byte {
	if err := sdk.VerifyAddressFormat(validator); err != nil {
		panic(sdkerrors.Wrap(err, "invalid validator address"))
	}
	return AppendBytes(GetValsetConfirmNoncePrefix(chainID, nonce), validator.Bytes())
}
...
func GetAttestationKey(chainID ChainID, eventNonce uint64, claimHash []byte) []byte {
	return AppendBytes(OracleAttestationKey, chainID.Bytes(), UInt64Bytes(eventNonce), claimHash)
}
...
func GetOutgoingTxPoolContractPrefix(chainID ChainID, contractAddress EthAddress) []byte {
	return AppendBytes(OutgoingTXPoolKey, chainID.Bytes(), contractAddress.GetAddress().Bytes())
}
...
func GetOutgoingTxPoolKey(chainID ChainID, fee InternalERC20Token, id uint64) []byte {
	amount := make([]byte, 32)
	amount = fee.Amount.BigInt().FillBytes(amount)
	return AppendBytes(OutgoingTXPoolKey, chainID.Bytes(), fee.Contract.GetAddress().Bytes(), amount, UInt64Bytes(id))
}

// GetOutgoingTxBatchContractPrefix returns the following format
// prefix     eth-contract-address
// [0x0][0xc783df8a850f42e7F7e57013759C285caa701eB6]
func GetOutgoingTxBatchContractPrefix(chainID ChainID, tokenContract EthAddress) []byte {
	return AppendBytes(OutgoingTXBatchKey, chainID.Bytes(), tokenContract.GetAddress().Bytes())
}
...
func GetOutgoingTxBatchKey(chainID ChainID, tokenContract EthAddress, nonce uint64) []byte {
	return AppendBytes(GetOutgoingTxBatchContractPrefix(chainID, tokenContract), UInt64Bytes(nonce))
}

// GetBatchConfirmNonceContractPrefix returns
// prefix           eth-contract-address                BatchNonce
// [0x0][0xc783df8a850f42e7F7e57013759C285caa701eB6][0 0 0 0 0 0 0 1]
func GetBatchConfirmNonceContractPrefix(chainID ChainID, tokenContract EthAddress, batchNonce uint64) []byte {
	return AppendBytes(BatchConfirmKey, chainID.Bytes(), tokenContract.GetAddress().Bytes(), UInt64Bytes(batchNonce))
}
...
func GetBatchConfirmKey(chainID ChainID, tokenContract EthAddress, batchNonce uint64, validator sdk.AccAddress) []byte {
	if err := sdk.VerifyAddressFormat(validator); err != nil {
		panic(sdkerrors.Wrap(err, "invalid validator address"))
	}
	return AppendBytes(GetBatchConfirmNonceContractPrefix(chainID, tokenContract, batchNonce), validator.Bytes())
}
...
// prefix              cosmos-validator
// [0x0][gravity1ahx7f8wyertuus9r20284ej0asrs085ceqtfnm]
func GetLastEventNonceByValidatorKey(chainID ChainID, validator sdk.ValAddress) []byte {
	if err := sdk.VerifyAddressFormat(validator); err != nil {
		panic(sdkerrors.Wrap(err, "invalid validator address"))
	}
	return AppendBytes(LastEventNonceByValidatorKey, chainID.Bytes(), validator.Bytes())
}

func GetDenomToERC20Key(chainID ChainID, denom string) []byte {
	return AppendBytes(DenomToERC20Key, chainID.Bytes(), []byte(denom))
}

func GetERC20ToDenomKey(chainID ChainID, erc20 EthAddress) []byte {
	return AppendBytes(ERC20ToDenomKey, chainID.Bytes(), erc20.GetAddress().Bytes())
}

func GetOutgoingLogicCallKey(chainID ChainID, invalidationId []byte, invalidationNonce uint64) []byte {
	return AppendBytes(KeyOutgoingLogicCall, chainID.Bytes(), invalidationId, UInt64Bytes(invalidationNonce))
}

func GetLogicConfirmNonceInvalidationIdPrefix(chainID ChainID, invalidationId []byte, invalidationNonce uint64) []byte {
	return AppendBytes(KeyOutgoingLogicConfirm, chainID.Bytes(), invalidationId, UInt64Bytes(invalidationNonce))
}

func GetLogicConfirmKey(chainID ChainID, invalidationId []byte, invalidationNonce uint64, validator sdk.AccAddress) []byte {
	if err := sdk.VerifyAddressFormat(validator); err != nil {
		panic(sdkerrors.Wrap(err, "invalid validator address"))
	}
	return AppendBytes(GetLogicConfirmNonceInvalidationIdPrefix(chainID, invalidationId, invalidationNonce), validator.Bytes())
}

// GetPastEthSignatureCheckpointKey returns the following key format
// prefix    checkpoint
// [0x0][ checkpoint bytes ]
func GetPastEthSignatureCheckpointKey(chainID ChainID, checkpoint []byte) []byte {
	return AppendBytes(PastEthSignatureCheckpointKey, chainID.Bytes(), []byte(convertByteArrToString(checkpoint)))
}
