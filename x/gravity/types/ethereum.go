...
const (
	...
	GravityDenomLen = len(GravityDenomPrefix) + len(GravityDenomSeparator) + ChainIDLen + ETHContractAddressLen
...
func (e ERC20Token) ToInternal() (*InternalERC20Token, error) {
	return NewInternalERC20Token(e.Amount, e.Contract, ChainID(e.ChainId))
}
...
func NewInternalERC20Token(amount sdk.Int, contract string, chainID ChainID) (*InternalERC20Token, error) {
...
func (i *InternalERC20Token) ValidateBasic() error {
	...
	if i.ChainID == "" {
		return sdkerrors.Wrap(err, "invalid chain id")
	}
...
func (i *InternalERC20Token) GravityCoin() sdk.Coin {
	return sdk.NewCoin(GravityDenom(i.Contract, i.ChainID), i.Amount)
}

// GravityDenom converts an EthAddress to a gravity cosmos denom
func GravityDenom(tokenContract EthAddress, chainID ChainID) string {
	return fmt.Sprintf("%s%s%s%s", GravityDenomPrefix, GravityDenomSeparator, strings.Repeat("0", ChainIDLen-len(chainID.String()))+chainID.String(), tokenContract.GetAddress().Hex())
}
...
func (i *InternalERC20Token) Add(o *InternalERC20Token) (*InternalERC20Token, error) {
	...
	if i.ChainID != o.ChainID {
		return nil, sdkerrors.Wrap(ErrMismatched, "cannot add two different tokens")
	}

	sum := i.Amount.Add(o.Amount) // validation happens in NewInternalERC20Token()
	return NewInternalERC20Token(sum, i.Contract.GetAddress().Hex(), i.ChainID)
}
